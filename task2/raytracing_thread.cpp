...
void thread_func(int id, queue<ColStartEnd> &data, mutex &mut,
Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int numOfSamples) {
	while (true) {
		ColStartEnd elem;
		{
...
int main(int argc, char **argv) {
    ...
    queue<ColStartEnd> data;
	for (int i = 0; i < viewPlaneResolutionY; i++) {
		auto col = new ColStartEnd();
		col->startY = i;
		col->endY= i+1;
		data.push(&col);
	}
    ...
    image.saveJPEG("raytracingXX_" + to_string(numberOfThreads) + ".jpg");
    return 0;
}